...
- vcf (required): Path to the phased genotype file in the gzipped vcf format. Note that currently missing genotypes are not supported. If you use Eagle2 for phasing, there shouldn't be missing genotype in the default setting.
- msp (required): Path to the whole genome solved local ancestry files in the msp format.
